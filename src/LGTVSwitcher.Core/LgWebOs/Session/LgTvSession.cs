...
public sealed class LgTvSession
{
    ...
    public async Task EnsureConnectedAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!_stateLoaded && _clientKeyStore is not null)
            {
                var state = await _clientKeyStore.GetStateAsync(cancellationToken).ConfigureAwait(false);
                _options.ClientKey ??= state.ClientKey;
                _options.PreferredTvUsn ??= state.PreferredTvUsn;
                _stateLoaded = true;
            }
            ...
            foreach (var candidate in candidates)
            {
                try
                {
                    ...
                    if (registrationResult.Status != LgTvRegistrationStatus.Completed)
                    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (parsedResponse.Status == LgTvRegistrationStatus.Completed)
