...
public sealed class LgTvResponseParser
{
    ...
    public LgTvRegistrationResponse ParseRegistrationResponse(string json)
    {
        ...
        {
            "registered" => LgTvRegistrationStatus.Completed,
            "response" when returnValue == true && !string.IsNullOrWhiteSpace(clientKey)
                => LgTvRegistrationStatus.Completed,
            "error" when string.IsNullOrWhiteSpace(envelope.Error) ||
                ...
                => LgTvRegistrationStatus.Pending,
            "error" => throw new LgTvRegistrationException($"LG TV registration failed: {json}"),
            _ => LgTvRegistrationStatus.Pending,
        };
